...
const RegisterPage = () => {
  ...
  const handleRegister = async () => {
    if (loading) return;
    const { fullName, email, phone, password, confirmPassword, role } = registerData;
          ...
          <div className="w-full md:w-1/2 p-8">
            ...
            <div className="space-y-4">
              ...
              <button
                type="button"
                onClick={handleRegister}
                className={`w-full bg-[#5C4033] hover:bg-[#8B5E3C] text-white py-2 font-semibold rounded-lg shadow-md hover:scale-105 transition-transform ${loading ? 'opacity-60 cursor-not-allowed' : ''}`}
                disabled={loading}
              >
